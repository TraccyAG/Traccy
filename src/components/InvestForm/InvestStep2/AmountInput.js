        ...
        saleContract.phaseCounter().then(
            counter => saleContract.phaseInfo(counter - 1 > 0 ? counter - 1 : 0).then(
                res => {
                    ...
                    const phaseTokenomics = {
                        total: ethers.utils.formatEther(res.amountTotal),
                        available: ethers.utils.formatEther(res.amountTotal.sub(res.amountSold))
                    };
    ...
    return (
        <div className="input-wrapper">
            ...
            {token}
                ...
                <span
                    className="balance">{`available in this phase : ${parseInt(phaseVolume.available)} / ${parseInt(phaseVolume.total)}`}</span>
            </div>
        </div>
    )
}
